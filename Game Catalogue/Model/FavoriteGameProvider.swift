...
class FavoriteGameProvider {
    ...
    func getAllFavoriteGame(completion: @escaping(_ games: [GameModel]) -> ()){
        ...
        taskContext.perform {
            ...
            do {
                ...
                for result in results {
                    ...
                    fetchRequestPlatform.predicate = NSPredicate(format: "game_id == \(result.value(forKey: "id") ?? 0)")
                    do {
                        ...
                        fetchRequestClip.predicate = NSPredicate(format: "game_id == \(result.value(forKey: "id") ?? 0)")
                        do {
                            if let resultClip = try taskContext.fetch(fetchRequestClip).first{
                                let clipTemp = Clip(clip: resultClip.value(forKey: "clip") as? String, preview: resultClip.value(forKey: "preview") as? String)
                                clip = clipTemp
                            }
                        ...
                        let fetchRequestShortScreenshot = NSFetchRequest<NSManagedObject>(entityName: "ShortScreenshotEntity")
                        fetchRequestShortScreenshot.predicate = NSPredicate(format: "game_id == \(result.value(forKey: "id") ?? 0)")
                        do {
                            let resultScreenshot = try taskContext.fetch(fetchRequestShortScreenshot)
                            for screenshot in resultScreenshot{
                                let screenshotTemp = ShortScreenshot(id: screenshot.value(forKey: "id") as? Int, image: screenshot.value(forKey: "image") as? String)
    ...
    func getFavoriteGame(_ id: Int, completion: @escaping(_ games: GameModel) -> ()){
        let taskContext = newTaskContext()
        taskContext.perform {
            let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: "GameEntity")
    ...
    func addToFavorite(gameDetail: GameModel, completion: @escaping() -> ()){
        let taskContext = newTaskContext()
    ...
    func addPlatform(gameDetail: GameModel){
        let parentPlatform:[ParentPlatform] = gameDetail.parent_platforms ?? [ParentPlatform]()
        ...
        for platform in parentPlatform {
            ...
            let entityPlatform = NSEntityDescription.entity(forEntityName: "PlatformGameEntity", in: managedObjectContext)
    ...
    func addScreenshot(gameDetail: GameModel){
        let shortScreenshot:[ShortScreenshot] = gameDetail.short_screenshots ?? [ShortScreenshot]()
        ...
        for screenshot in shortScreenshot {
            ...
            let entityPlatform = NSEntityDescription.entity(forEntityName: "ShortScreenshotEntity", in: managedObjectContext)
            let platformObj = NSManagedObject(entity: entityPlatform!, insertInto: managedObjectContext)
    ...
    func removeAllFavorite(completion: @escaping() -> ()) {
        let taskContext = newTaskContext()
        taskContext.perform {
    ...
    func removeFavorite(_ id: Int, completion: @escaping() -> ()){
        ...
        taskContext.perform {
            ...
            if let batchDeleteResultPlatform = try? taskContext.execute(batchDeleteRequestPlatform) as? NSBatchDeleteResult,
            ...
            if let batchDeleteResultScreenshot = try? taskContext.execute(batchDeleteRequestScreenshot) as? NSBatchDeleteResult,
