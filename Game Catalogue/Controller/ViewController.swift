...
//                    NSLog("Result" + String(decoding: data, as: UTF8.self))
                    ...
                    new.forEach { (result) in
                        let newData = GameModel(
                            ...
                            parentPlatforms: result.parentPlatforms ?? [ParentPlatform](),
                            clip: result.clip,
                            shortScreenshots: result.shortScreenshots ?? [ShortScreenshot]())
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, _ in
            guard let response = response as? HTTPURLResponse, let data = data else { return }
                if response.statusCode == 200 {
                    ...
                    if index < gameDataFiltered.count {
                        gameDataFiltered[index].description = detail?.description ?? ""
                    }
                } else {
                    NSLog("Something When Wrong " + String(response.statusCode))
                }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(
            ...
            if gameDataFiltered?.isEmpty == false && indexPath.row < gameDataFiltered.count {
                ...
                let parentPlatform: [ParentPlatform] = detailGame.parentPlatforms ?? [ParentPlatform]()
